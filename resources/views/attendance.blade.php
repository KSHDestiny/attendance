...
@section('CR')
    <article class="d-flex">
        <form action="{{ route('attendance.create') }}" method="POST">
            @csrf
            <button class="btn btn-warning mx-1">{{ __('attendance.createAll') }}</button>
        </form>
        <form action="{{ route('attendance.delete') }}" method="POST">
            @csrf
            <button class="btn btn-danger mx-1">{{ __('attendance.deleteAll') }}</button>
        </form>
    </article>
@endsection
...
<link rel="stylesheet" href="http://cdn.bootcss.com/toastr.js/latest/css/toastr.min.css">
    <article class="d-flex justify-content-between align-items-center">
        <section class="d-flex">
            <div class="form-group">
                <select id="department" class="form-select">
                    <option value="" selected disabled>{{ __('attendance.department') }}</option>
            ...
            <div>
                <select id="position" class="form-select ms-2">
                    <option value="" selected disabled>{{ __('attendance.position') }}</option>
        ...
        <section class="form-group d-flex">
            <input type="text" id="name" class="form-control" placeholder="{{ __('attendance.placeholder') }}">
            <button id="searchNameBtn" class="btn btn-success ms-2">{{ __('attendance.search') }}</button>
            <button onclick="location.reload();" class="btn btn-danger ms-2">{{ __('attendance.clear') }}</button>
        </section>
    </article>
            ...
            <table class="table table-light mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="d-none d-md-table-cell">{{ __('attendance.department') }}</th>
                        <th class="d-none d-md-table-cell">{{ __('attendance.location') }}</th>
                        <th class="d-none d-md-table-cell">{{ __('attendance.position') }}</th>
                    </tr>
                </thead>
            ...
            <table class="table table-hover mt-2 ms-2">
                <thead>
                    <tr>
                        ...
                        <th>{{ __('attendance.start') }}</th>
                        <th>{{ __('attendance.break') }}</th>
                        <th>{{ __('attendance.finish') }}</th>
                    </tr>
                </thead>
                    ...
                    <article class="modal fade" id="staticBackdrop{{ $attendance->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                ...
                                <h1 class="modal-title fs-5" id="staticBackdropLabel{{ $attendance->id }}">{{ __('attendance.editAttendance') }} ({{ $attendance->employee->name }})</h1>
                                ...
                                <section class="modal-body">
                                    <form action="{{ route('attendance.edit') }}" method="POST">
                                        ...
                                        <div class="my-2">
                                            <label for="status">{{ __('attendance.status') }}</label>
                                        ...
                                        <div class="my-2">
                                            <label for="finish">{{ __('attendance.finish') }}</label>
                                            <input type="time" name="finish" id="finish" value="{{ old('finish',$attendance->finish) }}" class="form-control">
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('attendance.close') }}</button>
                                            <button type="submit" class="btn btn-primary">{{ __('attendance.update') }}</button>
                                        </div>
                                    </form>
                                </section>
                            </div>
